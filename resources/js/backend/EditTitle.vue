        ...
        <div class="columns is-centered">
            <div class="column is-four-fifths">
                ...
                <div class="columns is-multiline">
                    ...
                    <div class="column is-3">
                        <div class="field">
                            <label class="label">Editor Rating</label>
                            <a v-for="ys in yellow" href="#" :key="'yellow_' + ys" class="mx-0 px-0"
                               @click="delStar(ys)">
                    ...
                    <div class="column is-3">
                        <div class="field">
                            ...
                            <a v-for="ys in i_yellow" href="#" :key="'iyellow_' + ys" class="mx-0 px-0"
                               @click="delIStar(ys)">
                                <span class="icon has-text-orange mx-0 px-0"><i class="fas fa-star"></i></span>
                            </a>
                            <a v-for="ws in i_white" href="#" :key="'iwhite_'+ ws" class="mx-0 px-0" @click="addIStar(ws)">
                                <span class="icon has-text-orange mx-0 px-0"><i class="far fa-star"></i></span>
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="column is-4">
                        <button class="button is-primary is-fullwidth" @click="editTitle">ENVIAR</button>
                    </div>
                    <div class="column is-4"></div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-content">
                    <div class="content has-text-centered">
                        <p>{{ header }} editado com sucesso</p>
                        <button class="button is-primary" autofocus @click="closeModal">FECHAR</button>
                    </div>
                </div>
            </div>
        </b-modal>
...
export default {
    ...
    methods: {
        ...
        delIStar(ys) {
            ...
            this.title.imdb_rating = this.i_yellow
        },
        addIStar(ws) {
            console.log('ADD', ws)
            this.i_yellow += ws
            this.i_white = 5 - this.i_yellow
            this.title.imdb_rating = this.i_yellow
        },
        editTitle() {
            axios.put(`/api/${this.table}/update`, this.title).then(response => {
        ...
        closeModal() {
            ...
            if(this.table === 'movies') {
                this.$router.push({ name: 'admin-movies', params: { channel: this.channel }})
            } else {
                this.$router.push({ name: 'admin-series', params: { channel: this.channel }})
            }
        }
    },
    beforeMount() {
        ...
        this.i_yellow = this.title.imdb_rating
        this.i_white = 5 - this.title.imdb_rating
